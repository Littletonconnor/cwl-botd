...
- [x] **6.1 Unit tests for all collectors**
...
- [x] **6.2 Unit tests for all detectors**
...
- [x] **6.3 Unit tests for scoring engine**
...
- [x] **6.5 Performance benchmarks** (CI step deferred)
  - [x] Measure collection time (target: <5ms)
  - [x] Measure detection time (target: <2ms)
  - [x] Memory usage profiling
  - [x] Bundle size tracking (target: <30KB gzipped ESM, <120KB raw)
  - [ ] Add benchmark CI step to prevent regressions (deferred to Phase 8)

- [x] **6.6 Simulated evasion testing**
  - [x] Create mock "stealth bot" fingerprints
  - [x] Test detection of UA spoofing
  - [x] Test detection of property override via Proxy
  - [x] Test detection of canvas noise injection
  - [x] Test detection of WebGL renderer spoofing
...
- [x] **6.7 Fix `performancePrecision` detector false positives**
  - Replaced two-sample approach with multi-sample strategy (6 samples across ~25ms of CPU work)
  - Frozen clock detection: flags only when ALL samples are identical (score 0.6)
  - Integer-only detection: flags when all values are integers AND total elapsed ≤ expected (score 0.5) — avoids false-positiving on Firefox's legitimate 1ms rounding
  - Added Firefox 1ms-rounding regression test to prevent future false positives
